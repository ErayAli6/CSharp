...
namespace LINQ
{
    internal class Program
    {
        ...
        {
            /*This sample uses Range to generate a sequence of numbers from 0 to 99 " 
              that is used to find which numbers in that range are odd and even.*/
            public void GenerateSequenceOfNumbersInRangeAndPrintOddandEvenNumbers()
            {
                var numbers =
                    from n in Enumerable.Range(0, 99)
            ...
            /* Generate a sequence which containts number 1, 10 times */
            public void GenerateRepeatSequence()
            {
            ...
            /*Check sequences are equal*/
            public void CheckSequenesAreEqual()
            {
                int[] arr1 = { 1, 2, 3, 4, 5 };
                int[] arr2 = { 1, 2, 3, 4, 5 };
            ...
            /* Prnt min and max number in array */
            public void PrintMinMaxNumberInArray()
            {
                int[] numbers = { 5, 4, 1, 3, 9, 8, 6, 7, 2, 0 };

                Console.WriteLine($"The minimum number is {numbers.Min()}.");
                Console.WriteLine($"The minimum number is { numbers.Max()}.");
            }

            /* Get the length of the longest word in an array */
            public void PrintLongestWordInArray()
            {
        ...
        public class Samlpes
        {
            ...
            /*Find product by ProductID, and return first. You have to be sure that such a product with this Id exists 
              otherwise it will throw an exception */
            public Product GetFirstProductById(int id)
            {
                return productList.Where(x => x.ProductID == id).First();
            }

            /* return product by index in the list */
            ...
            public Product GetProductByIndexOrNull(int index)
            {
            ...
            {
                var productsInStock = productList.Where(x => x.UnitsInStock > 0).ToList();
            ...
            {
                return productList.Where(x => x.Category == category);
            }

            /* Use Distinct to find the unique Category names */
            public IEnumerable<string> GetCategoryNames()
            {
                var categoryNames = productList.Select(x => x.Category).Distinct();
                return categoryNames;
            }

            /* Print all Products  grouped by Category */
            public void PrintProductsGroupByCategory()
            {
                var productGroups = productList.GroupBy(x => x.Category).Select(x => new { Category = x.Key, Products = x });
            ...
            /* Check if product with passed name exists */
            public bool CheckProductExists(string productName)
            {
                return productList.Any(x => x.ProductName == productName);
            }

            /* returns the top 10 products order by in stock descending */
            public IEnumerable<Product> TakeTop10ProductsInStock()
            {
                var top10 = productList.OrderByDescending(x => x.UnitsInStock).Take(10);

                return top10;
            }

            /*First order by units in stock, after that by prodcutName */
            public void PrintProductsOrderByUnitsInStockThenByProductName()
            {
                var orderedList = productList.OrderBy(x => x.UnitsInStock).ThenBy(x => x.ProductName);
            ...
            public void ListOfCategoriesAndProductsCount()
            {
                var categoryCounts =
